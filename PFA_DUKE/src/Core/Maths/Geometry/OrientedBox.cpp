...
namespace Core::Maths::Geometry
{
	...
	OrientedBox::OrientedBox(BoxCollider* box)
	{
		if (!box)
			return;
		...
		Mat4 mat(box->GetTRS());
		m_i = Vec3(mat * Vec4(1, 0, 0, 0));
		m_j = Vec3(mat * Vec4(0, 1, 0, 0));
		m_k = Vec3(mat * Vec4(0, 0, 1, 0));

		m_box = Box(box->Position(), m_i.Length(), m_j.Length(), m_k.Length());
